...
export const ResultsPage = ({ discussionId, onBack }: {
  ...
  const getUsersForGroup = (group: AIGroup) => {
    if (!discussion || discussion.is_anonymous) {
      return [];
    }

    return group.members
      .map((memberIndex) => {
        const response = responses[memberIndex];
        if (!response || response.is_anonymous) return null;
        return response.users?.name || 'Unknown';
      })
                  ...
                  {aiAnalysis.groups.map((group, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="text-sm text-gray-600 mb-2">{group.criteria}</p>
                        {groupUsers.length > 0 && (
                          <div className="mt-3 pt-3 border-t border-gray-200">
                            <p className="text-xs font-medium text-gray-500 mb-1">Participants:</p>
                            <div className="flex flex-wrap gap-1">
                              {groupUsers.map((userName, userIndex) => (
                                <span
                                  key={userIndex}
