...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { question, responses, likeCounts }: AnalysisRequest = await req.json();
    console.log('Received request - Question:', question);
    console.log('Number of responses:', responses?.length);
    console.log('Like counts provided:', !!likeCounts);
    ...
    const formattedResponses = responses
      .map((response, index) => {
        const likeCount = likeCounts && likeCounts[index] !== undefined ? likeCounts[index] : 0;
        return `${index + 1}. ${response}${likeCount > 0 ? `, ${likeCount} likes` : ''}`;
      })
      .join('\n');
